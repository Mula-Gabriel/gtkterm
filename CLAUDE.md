...
**Self-update (`update.c` + `data/gtkterm-update.sh`):** the running binary knows its own commit because `meson.build` bakes `git rev-parse HEAD` into `GIT_REVISION`. `update_check()` runs `git ls-remote <update_url> HEAD` asynchronously (`g_subprocess`) and compares the remote SHA against `GIT_REVISION` — silent at startup unless a newer revision exists (skippable), always reporting when triggered manually from the About dialog. Accepting launches `data/gtkterm-update.sh` (installed under `PACKAGE_DATA_DIR`, also baked by meson) inside a detected terminal emulator; the script clones/updates into `$XDG_CACHE_HOME/gtkterm/src`, installs build deps, builds, `pkexec`-installs, and restarts the app by PID. The repo URL and install prefix come from `config.update_url`/`config.update_prefix` (`.gtktermrc`, editable in the About dialog; defaults to the GitHub fork and `/usr/local`).

**Other entry points:** `gtkterm.c` (`main`, wiring order; schedules the startup update check via `g_idle_add`), `interface.c` (the large GTK GUI — menus, VTE widget, view switching, shortcuts, About dialog), `device_monitor.c` (GUdev serial-device hotplug), `user_signals.c` (SIGUSR1/2 → open/close port), `cmdline.c` (CLI options incl. `--transport/--host/--tcp-port`), `search.c`, `logging.c`, `files.c`, `i18n.c` (gettext; translations in `po/`).
